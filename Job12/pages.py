...
class Answer_flag(Page):
    ...
    def before_next_page(self):
        ...
        if self.player.flag1 == "Japan":
            #Hvor mye poeng man får for å ha rett
            self.player.this_round_point = self.player.this_round_point + 5
            
        if self.player.flag2 == "Israel":
            self.player.this_round_point = self.player.this_round_point + 5
            
        if self.player.flag3 == "Italy":
            self.player.this_round_point = self.player.this_round_point + 5
            
        if self.player.flag4 == "Spain":
            self.player.this_round_point = self.player.this_round_point + 5
            
        if self.player.flag5 == "USA":
            self.player.this_round_point = self.player.this_round_point + 5
            
        if self.player.flag6 == "Sweden":
            self.player.this_round_point = self.player.this_round_point + 5
            
        if self.player.flag7 == "United Kingdom":
            self.player.this_round_point = self.player.this_round_point + 5
