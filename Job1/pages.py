...
class Typeracer(Page):
    ...
    def before_next_page(self):
        ...
        my_string = "They discussed its ramifications. Possession of grain alcohol was a not to be missed opportunity to dilute genuine liquor. Such a big ship could carry well over a hundred thousand barrels five hundred railroad tank cars easily stretched to fifty million bottles."
...
class Got_Audited(Page):
    ...
    def is_displayed(self):
        if self.player.random_num <= 5 and self.player.declare != self.player.this_round_point:
            ...
            #self.player.payoff = self.player.payoff + (self.player.declare * 0.6) + (self.player.this_round_point -self.player.declare)
            return False
